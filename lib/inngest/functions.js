...
export const triggerRecurringTransactions = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    return { triggeres: recurringTransactions.length };
  }
);
...
export const processRecurringTransaction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run("process-transaction", async () => {
      ...
      if (!transaction || !isTrasactionDue(transaction)) return;
...
function isTrasactionDue(transaction) {
...
async function generateFinancialInsights(stats, month) {
  ...
  try {
    ...
    return [
      "Your highest expense category this month might need some attention.",
...
const getMonthlyStats = async (userId, month) => {
  ...
  const endDate = new Date(month.getFullYear(), month.getFullYear() + 1, 0);
